...
function sanitizeError(value: unknown): string {
  ...
  return text
    .replace(/([?&](?:key|token|api_key|apikey|authorization|secret)=)[^&\s]+/gi, "$1[redacted]")
...
export function ProviderModelTestDialog({
  ...
  const [liveCandidates, setLiveCandidates] = useState<ProviderModelCandidate[]>(
    [],
  );
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-lg" zIndex="top">
        <DialogHeader className="relative pr-14">
          ...
          <DialogDescription>
            {provider?.name ?? "Provider"} · {t("provider.modelTest.description", {
              defaultValue: "Send a fixed non-streaming hi request to this provider.",
            })}
          </DialogDescription>
          <DialogClose asChild>
            <Button
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
              ...
              <div className="space-y-1">
                <p className="font-medium">
                  {t("provider.modelTest.billingWarning", {
                    defaultValue: "This sends one billable request to the selected provider.",
                  })}
                </p>
                <p className="text-xs opacity-90">
                  {t("provider.modelTest.hiDisclosure", {
                    defaultValue: 'The request body is fixed to "hi" and does not stream.',
                  })}
                </p>
              </div>
            </div>
          ...
          {!eligible && (
            <div role="alert" className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm">
              {t("provider.modelTest.unavailable", {
                defaultValue: "Model testing is not available for this provider.",
              })}
            </div>
          )}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              <label className="text-sm font-medium" htmlFor="provider-model-test-select">
            ...
            >
              <SelectTrigger id="provider-model-test-select" aria-label={t("provider.modelTest.model", { defaultValue: "Model" })}>
                <SelectValue
                    ...
                    <span className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {candidate.source === "configured"
                          ? t("provider.modelTest.configured", { defaultValue: "configured" })
                          : t("provider.modelTest.live", { defaultValue: "live" })}
                      </span>
            ...
            {candidates.length === 0 && (
              <p className="text-xs text-muted-foreground">
                {t("provider.modelTest.noCandidatesHint", {
                  defaultValue: "Add a model to this provider before testing it.",
                })}
              </p>
            )}
            {refreshError && (
              <p role="alert" className="text-xs text-amber-700 dark:text-amber-300">
                {t("provider.modelTest.refreshFailed", {
                  defaultValue: "Live model refresh failed; configured models are still available.",
                })}{" "}{refreshError}
              </p>
            )}
          </div>
          ...
          {(isStarting || status === "running" || status === "retrying") && (
            <div className="flex items-center gap-2 rounded-md border p-3 text-sm">
              ...
              {status === "retrying"
                ? t("provider.modelTest.retrying", { defaultValue: "Retrying…" })
                : t("provider.modelTest.running", { defaultValue: "Testing…" })}
          ...
          {status === "succeeded" && (
            <div className="rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm">
              <div className="flex items-center gap-2 font-medium text-emerald-700 dark:text-emerald-300">
                ...
                {t("provider.modelTest.success", { defaultValue: "Model test succeeded" })}
          ...
          {(status === "failed" || status === "cancelled" || testError) && (
            <div role="alert" className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm">
              <div className="flex items-center gap-2 font-medium text-destructive">
                ...
                {status === "cancelled"
                  ? t("provider.modelTest.cancelled", { defaultValue: "Test cancelled" })
                  : t("provider.modelTest.failed", { defaultValue: "Model test failed" })}
              </div>
              <p className="mt-1 break-words text-xs">{sanitizeError(result?.message ?? testError)}</p>
              {result?.errorCategory && (
                <p className="mt-1 text-xs text-muted-foreground">{result.errorCategory}</p>
              )}
            </div>
          )}
          </div>
        ...
        <DialogFooter>
          {isTesting && (
            <Button type="button" variant="outline" onClick={() => void cancelTest()} disabled={isCancelling}>
              {isCancelling && <Loader2 className="h-4 w-4 animate-spin" />}
              {t("provider.modelTest.cancel", { defaultValue: "Cancel" })}
            </Button>
          )}
          <Button type="button" onClick={handleTest} disabled={!canTest || busy}>
